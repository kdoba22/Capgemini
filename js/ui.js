...
export function applyDefaultRecent3() {
  ...
  state.filteredMonthIndex = Number(m) - 1; // subtract 1 since onth is zero based
}
...
export function renderCustomerTable() {
  ...
  const customers = Array.from(
    new Set(state.allTxs.map((t) => t.customerId))
  ).sort();
...
export function renderCustomerSummary() {
  ...
  const txs = filterByYearMonth(state.allTxs, year, null).filter(
    (t) => t.customerId === cid
  );
...
export function renderTransactionsForSelectedMonth() {
  ...
  const monthTxs = filterByYearMonth(state.allTxs, year, mIdx).filter(
    (t) => t.customerId === cid
  );
  ...
  for (const t of monthTxs.sort(
    (a, b) => new Date(a.date) - new Date(b.date)
  )) {
    const pts = calcPoints(t.amount);
    const li = document.createElement("li");
    const d = new Date(t.date);
    li.innerHTML = `<div><strong>${d.toLocaleDateString()}</strong> — ${money(
      t.amount
    )} — <span class="tag">${pts} pts</span></div>
